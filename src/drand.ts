...
export const getFutureDrandRound = async () => {
  ...
  const pickTime = new Date("2024-10-08").setHours(9, 0, 0, 0);
  ...
  console.log(`Latest round: ${latestRoundNumber}`);
  console.log(`Round: ${round}`);
  const futureRound = latestRoundNumber + round;
  console.log(`Future round: ${futureRound}`)
  return futureRound;
}