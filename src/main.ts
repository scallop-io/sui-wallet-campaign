...
// DateTime: 2024-10-08 02:00:00 AM UTC
const PICKED_ROUND = 4430725;
...
async function main() {
  ...
  if(isTest) {
    ...
    addresses = JSON.parse(fs.readFileSync('sui-wallet-campaign.json', 'utf-8')) as string[];
  }
...
async function generateHashResults(addresses: string[], selectedRound: number) {
  ...
  addresses.forEach((address) => {
    ...
    const hash = blake2b(output.length, undefined, undefined, Buffer.from("sui-wallet-camp1")).update(input).digest('hex');
    hashedResults.push({
      address,
